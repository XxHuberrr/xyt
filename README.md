...
xy+t 是一个基于 Web 的四维视频切片播放器：视频帧写入 WebGL2 `sampler2DArray`，每张 XY 帧平面沿 Z 轴按时间排列，拖拽画布可旋转时间体，并在无边界的流式深空中进行 WASD / Space / Ctrl 导航。视觉原设计参考：Instagram 艺术家 [@bradleytangonan](https://www.instagram.com/bradleytangonan/)。本项目是独立实现，代码以 MIT 许可证开源；MIT 许可证不授予第三方视频素材的使用权。
...
- 视频载入后，播放器按左侧「采样帧率」从全时长写入 XY 时间切片，默认 20 FPS，可调范围为 1–50 FPS（受显卡可用层数保护），松开滑杆会重新建立时间体。处理分辨率可选 720p 或 1080p；高帧率会按 GPU 预算自动降低实际纹理尺寸以保持可运行，原视频帧不会直接写入纹理。首层会跳过浏览器解码产生的黑色首帧并选取第一个有效画面。默认不自动旋转，拖拽画布旋转，滚轮调整观察距离；播放或拖动时间轴时，当前时间帧会以高亮叠加显示但不会遮挡前景切片，立方体上方侧边和深度方向显示时间刻度与当前轨迹，其他切片保留边缘羽化过渡。
- 画布是一个持续流式的无限四维空间：相机坐标没有人为边界，跨越世界 cell 时会生成新的确定性星场参照；时间体是空间中的一块可旋转时间材质，离开它后仍可继续在深空中移动。Shift 冲刺会平滑提高速度并拉宽视野，星场以加法光方式保持稀疏，不用大量片状几何填满画面。
